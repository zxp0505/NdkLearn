...
using namespace std;
 char _PACKNAME[] = "com/zxp/ndklearn/bean";
...
void register_inner_class(JNIEnv *env) {
     std::string _strClass = string(_PACKNAME)+"/DataBean$Inner";
    int ret = find_class(env, _strClass.c_str(), &m_inner_block.clazz);
    if (ret != 0) {
...
void register_data_bean_class(JNIEnv *env) {
    string dataBeanclass = string(_PACKNAME)+"/DataBean";
    int ret = find_class(env, dataBeanclass.c_str(), &m_data_bean_block.clazz);//"com/afei/jnidemo/DataBean"
    if (ret != 0) {
    ...
    string innerClassName = "L"+string(_PACKNAME)+"/DataBean$Inner;";
    get_field(env, &clazz, "mInner",innerClassName.c_str() , &m_data_bean_block.inner);//"Lcom/afei/jnidemo/DataBean$Inner;"  "Lcom/zxp/ndklearn/bean/DataBean$Inner;"
